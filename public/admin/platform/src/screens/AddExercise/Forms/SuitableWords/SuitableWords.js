...
function SuitableWords({ props }) {
            ...
            <Accordion key={item.id} className="border p-3">
                <Card>
                    <Accordion.Toggle as={Card.Header} eventKey={item.id}>
                        {item.sentence ? item.sentence : 'اضغط لاضافة سؤال'}
                    </Accordion.Toggle>
                            ...
                            <Row>
                                <Col md="6">
                                    <Form.Group>
                                        ...
                                        <Form.Control onChange={e => handleSentenceChange(e.target.value, item.id)} value={item.sentence} />
                                    </Form.Group>
                                </Col>
                                <Col md="6">
    ...
    return (
        <>
            ...
            <a className="text-primary cursor-pointer" onClick={() => setItems([...items, { id: Date.now(), sentence: '', choices: [] }])}>
                <i className="fa fa-plus"></i>
