...
import SuitableWords from './SuitableWords'

export class SuitableWordsIndex extends Component {
    ...
    async handleSubmit(data) {
        ...
        let route = this.state.mode == 'create' ? `${routes.suitableWords.create}/${this.props.match.params.chapter}` : `${routes.suitableWords.edit}/${this.state.data.book_content_id}`
    ...
    render() {
        return (
            <Layout title={labels.suitable_words}>{this.renderTemplate()}</Layout>
        )
    }
}
...
export default connect(null, mapDispatchToProps)(SuitableWordsIndex)
