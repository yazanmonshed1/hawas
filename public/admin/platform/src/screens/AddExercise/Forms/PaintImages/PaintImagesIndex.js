...
import PaintImages from './PaintImages'

export class PaintImagesIndex extends Component {
    ...
    async handleSubmit(data) {
        ...
        let route = this.state.mode == 'create' ? `${routes.paintingImage.create}/${this.props.match.params.chapter}` : `${routes.paintingImage.edit}/${this.state.data.contents.book_content_id}`
    ...
    render() {
        return (
            <Layout title={labels.painting}>{this.renderTemplate()}</Layout>
        )
    }
}
...
export default connect(null, mapDispatchToProps)(PaintImagesIndex)
