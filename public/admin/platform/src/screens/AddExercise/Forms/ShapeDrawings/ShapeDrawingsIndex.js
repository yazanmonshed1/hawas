...
import ShapeDrawings from './ShapeDrawings'

export class ShapeDrawingsIndex extends Component {
    ...
    async handleSubmit(data) {
        ...
        let route = this.state.mode == 'create' ? `${routes.shapeDrawings.create}/${this.props.match.params.chapter}` : `${routes.shapeDrawings.edit}/${this.state.data.contents.book_content_id}`
    ...
    render() {
        return (
            <Layout title={labels.shape_drawings}>{this.renderTemplate()}</Layout>
        )
    }
}
...
export default connect(null, mapDispatchToProps)(ShapeDrawingsIndex)