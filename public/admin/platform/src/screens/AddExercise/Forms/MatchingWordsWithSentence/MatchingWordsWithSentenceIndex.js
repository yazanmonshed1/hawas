...
import MatchingWordsWithSentence from './MatchingWordsWithSentence'

export class MatchingWordsWithSentenceIndex extends Component {
    ...
    async handleSubmit(data) {
        ...
        let route = this.state.mode == 'create' ? `${routes.matchWordsToSentences.create}/${this.props.match.params.chapter}` : `${routes.matchWordsToSentences.edit}/${this.state.data.book_content_id}`
    ...
    renderTemplate() {
        ...
        switch (status) {
            case 'success':
                return <MatchingWordsWithSentence props={{
    ...
    render() {
        return (
            <Layout title={labels.matching_words_to_sentences}>
                {this.renderTemplate()}
            </Layout>
        )
    }
...
export default connect(null, mapDispatchToProps)(MatchingWordsWithSentenceIndex)
